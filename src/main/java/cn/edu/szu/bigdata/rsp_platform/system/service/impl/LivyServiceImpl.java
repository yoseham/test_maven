...
import cn.edu.szu.bigdata.rsp_platform.common.Constants;
import cn.edu.szu.bigdata.rsp_platform.common.eum.SparkJobState;
import cn.edu.szu.bigdata.rsp_platform.common.utils.HttpUtils;
import cn.edu.szu.bigdata.rsp_platform.common.utils.PropertiesUtil;
import cn.edu.szu.bigdata.rsp_platform.system.model.SparkJob;
...
public class LivyServiceImpl implements LivyService {
    ...
    private static String SPARK_HISTORY_URL="";
    ...
    public LivyServiceImpl() {
        try {
            ...
            SPARK_HISTORY_URL = String.valueOf(properties.get("SPARK_HISTORY_URL"));
        } catch (IOException e) {
            logger.error("请检查配置文件，找不到 Livy URL或者SPARK_HISTORY_URL");
            e.printStackTrace();
        }
    }
     ...
     * @return jobJson
     */
    private String parse2Json(SparkJob job) {
        // 过滤器，把默认值的参数剔除掉
        ...
        String result= JSON.toJSONString(job);
        return result;
    }
}